...
This skill switches ClawCode's container runtime from Docker to Apple Container (macOS-only). It uses the skills engine for deterministic code changes, then walks through verification.
...
Read `.clawcode/state.yaml`. If `convert-to-apple-container` is in `applied_skills`, skip to Phase 3 (Verify). The code changes are already in place.
...
If `.clawcode/` directory doesn't exist yet:
...
- Records the application in `.clawcode/state.yaml`
...
```bash
echo '{}' | container run -i --entrypoint /bin/echo clawcode-agent:latest "Container OK"
```
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.clawcode
```
