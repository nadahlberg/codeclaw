...
Adds Parallel AI MCP integration to ClawCode for advanced web research capabilities.
...
- **Non-blocking Design** - Uses ClawCode scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. ClawCode already set up and running
...
Add Parallel AI MCP servers after the clawcode server:
```typescript
const mcpServers: Record<string, any> = {
  clawcode: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__clawcode__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__clawcode__send_message
   4. Use mcp__clawcode__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo clawcode-agent:latest "Container OK"
```
...
```bash
npm run build
launchctl kickstart -k gui/$(id -u)/com.clawcode  # macOS
# Linux: systemctl --user restart clawcode
```
...
# Linux: systemctl --user status clawcode
```
...
- Check task runs: `tail -f logs/clawcode.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.clawcode` (macOS) or `systemctl --user restart clawcode` (Linux)
