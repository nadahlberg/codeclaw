...
# Fetch upstream ClawCode and extract to a temp directory.
...
if git remote get-url upstream &>/dev/null; then
  ...
  if echo "$ORIGIN_URL" | grep -q "nadahlberg/clawcode"; then
    REMOTE="origin"
  fi
fi

if [ -z "$REMOTE" ]; then
  echo "No upstream remote found. Adding upstream → https://github.com/nadahlberg/clawcode.git"
  git remote add upstream https://github.com/nadahlberg/clawcode.git
  REMOTE="upstream"
fi
...
TEMP_DIR=$(mktemp -d /tmp/clawcode-update-XXXX)
